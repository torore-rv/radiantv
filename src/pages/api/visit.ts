import { NextApiRequest, NextApiResponse } from 'next';
import pool from '@lib/db'; // DB 연결을 위한 pool

// 방문 카운트를 업데이트하거나 가져오는 API
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const { sessionId } = req.body;

    if (req.method === 'GET') {
        // sessionId가 없는 경우 - 방문자 수만 반환
        if (!sessionId) {
            try {
                // 오늘 방문한 사람 수
                const todayVisitorsResult = await pool.query(
                    `SELECT COUNT(*)
                 FROM visit
                 WHERE last_visited::date = CURRENT_DATE;`
                );
                const todayVisitors = parseInt(todayVisitorsResult.rows[0].count, 10);

                // 총 방문자 수
                const totalVisitorsResult = await pool.query('SELECT COUNT(*) FROM visit');
                const totalVisitors = parseInt(totalVisitorsResult.rows[0].count, 10);

                res.status(200).json({ todayVisitors, totalVisitors });
            } catch (error) {
                console.error('Error fetching visit counts:', error);
                res.status(500).json({ error: 'Failed to fetch visit counts' });
            }
        }

        // sessionId가 있는 경우 - 오늘 방문했는지 확인
        else {
            try {
                // 세션 ID로 오늘 방문 여부 확인
                const visitResult = await pool.query(
                    `SELECT COUNT(*) 
                 FROM visit 
                 WHERE session_id = $1
                   AND last_visited::date = CURRENT_DATE`,
                    [sessionId]
                );

                // 오늘 방문했으면 1, 아니면 0
                const visitedToday = parseInt(visitResult.rows[0].count, 10) > 0;

                res.status(200).json({ visitedToday });
            } catch (error) {
                console.error('Error checking visit for session:', error);
                res.status(500).json({ error: 'Failed to check visit for session' });
            }
        }
    }

    // POST 요청: 방문 카운트 업데이트
    if (req.method === 'POST') {
        try {
            // 세션 ID를 사용해 방문 기록을 업데이트하거나 새로 추가
            const visitedToday = await isVisitedToday(sessionId); // 오늘 방문했는지 확인

            if (visitedToday) {
                return res.status(200).json({ message: 'Already visited today' });
            }

            // 방문 카운트 증가
            await incrementVisitCount(sessionId);

            res.status(200).json({ message: 'Visit count updated successfully' });
        } catch (error) {
            console.error('Error updating visit count:', error);
            res.status(500).json({ error: 'Failed to update visit count' });
        }
    }
}

// 오늘 방문했는지 확인하는 함수
async function isVisitedToday(sessionId: string) {
    const result = await pool.query(
        `SELECT last_visited
         FROM visit
         WHERE session_id = $1
           AND last_visited::date = CURRENT_DATE`, // 날짜만 비교
        [sessionId]
    );
    return result.rows.length > 0;  // 오늘 방문한 기록이 있으면 true 반환
}


// 방문 카운트 업데이트 함수
async function incrementVisitCount(sessionId: string) {
    const currentCount = await getVisitCount(sessionId);

    if (currentCount === null) {
        // 방문 기록이 없다면 새로 삽입
        await pool.query(
            `INSERT INTO visit (session_id, visit_count, last_visited)
             VALUES ($1, 1, CURRENT_TIMESTAMP)`,
            [sessionId]
        );
    } else {
        // 방문 기록이 있으면 방문 횟수만 증가
        await pool.query(
            `UPDATE visit
             SET visit_count = visit_count + 1, last_visited = CURRENT_TIMESTAMP
             WHERE session_id = $1`,
            [sessionId]
        );
    }
}

// 방문 횟수를 가져오는 함수
async function getVisitCount(sessionId: string) {
    const result = await pool.query('SELECT visit_count FROM visit WHERE session_id = $1', [sessionId]);
    console.log('Full query result:', result);  // 전체 결과 출력
    return result.rows[0]?.visit_count || 0;  // 방문 횟수가 없으면 0을 반환
}